...
const Home = () => {
  ...
  const isItOnFavorite = data.map((item) =>
    item.city.toUpperCase().includes(citySearch)
  );
  console.log(isItOnFavorite);
  const dispatch = useDispatch();
  useEffect(() => {}, [citySearch]);
                ...
                <CardActionArea>
                  <CardContent>
                    ...
                    >
                      <h1>{citySearch}</h1>
                      <br />
                      <p style={{fontSize:"10px"}}>{item.Temperature.Maximum.Unit}</p>
                      <p >{item.Temperature.Maximum.UnitType}</p><br />
                      {item.Date}
                    </Typography>
                  </CardContent>
                </CardActionArea>
